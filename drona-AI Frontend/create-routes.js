...
const basePath = path.join(__dirname, 'src', 'app', '(platform)');
...
Object.entries(routes).forEach(([group, paths]) => {
  paths.forEach(routePath => {
    const fullDir = path.join(basePath, group, routePath);
    if (!fs.existsSync(fullDir)) {
      fs.mkdirSync(fullDir, { recursive: true });
    }
    const filePath = path.join(fullDir, 'page.tsx');
    const title = routePath.split('/').pop().replace(/-/g, ' ');
