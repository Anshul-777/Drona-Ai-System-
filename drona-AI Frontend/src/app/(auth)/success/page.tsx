...
const LINES = [
  { text: "Authentication Confirmed.", delay: 0,    speed: 45, style: "tagline" },
  { text: "DRONA AI",                   delay: 1200,  speed: 80, style: "brand"   },
  { text: "Loading your workspace...",   delay: 2800, speed: 30, style: "body" },
];
...
export default function SuccessPage() {
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      setExiting(true);
      setTimeout(() => router.push("/platform"), 650);
    }, 4500); // 2800 delay + 1000 for typing + 700 pause
  }, [router]);
    ...
    <div className="relative min-h-screen w-full flex flex-col overflow-hidden"
      ...
      <div className={`relative z-10 flex justify-between items-center px-8 sm:px-14 py-6 transition-all duration-700 ${mounted ? "opacity-100" : "opacity-0"}`}>
        <Link href="/" className="flex items-center gap-3 group">
          <div className="w-8 h-8 bg-black rounded-xl flex items-center justify-center shadow-lg border border-white/20">
            <span className="text-white font-serif font-bold text-base">D</span>
          </div>
          <span className="font-serif font-black text-xl tracking-tighter text-black drop-shadow-sm">
